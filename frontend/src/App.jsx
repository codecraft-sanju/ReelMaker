...
import { 
  ...
  Music, Crown, Check, X, Zap, Palette, LayoutTemplate, 
  ChevronLeft, Copy, AlignLeft, AlignCenter, AlignRight, Share2, Mic,
  Library, Sparkles, Eye
} from 'lucide-react';
...
const PRO_TEMPLATES = [
  {
    ...
    frames: [
      { id: 101, text: "Stop Waiting.", image: "", duration: 1.5, theme: THEMES[0], animation: ANIMATIONS[0].id, align: 'center', fontSize: 'large', aiVoice: false },
      { id: 102, text: "No one is coming to save you.", image: "", duration: 3, theme: THEMES[0], animation: ANIMATIONS[0].id, align: 'center', fontSize: 'medium', aiVoice: true },
      { id: 103, text: "BUILD IT YOURSELF.", image: "", duration: 2, theme: THEMES[2], animation: ANIMATIONS[0].id, align: 'center', fontSize: 'huge', aiVoice: true }
    ]
  },
  {
    id: 't2',
    ...
    frames: [
      { id: 201, text: "Here is a secret...", image: "", duration: 2, theme: THEMES[4], animation: ANIMATIONS[1].id, align: 'center', fontSize: 'medium', aiVoice: false },
      { id: 202, text: "Most people fail because they quit too early.", image: "", duration: 3.5, theme: THEMES[4], animation: ANIMATIONS[5].id, align: 'center', fontSize: 'small', aiVoice: true },
      { id: 203, text: "Don't be most people.", image: "", duration: 2.5, theme: THEMES[4], animation: ANIMATIONS[1].id, align: 'center', fontSize: 'large', aiVoice: true }
    ]
  },
  {
    id: 't3',
    ...
    frames: [
      { id: 301, text: "GAME OVER?", image: "", duration: 1, theme: THEMES[6], animation: ANIMATIONS[3].id, align: 'center', fontSize: 'large', aiVoice: false },
      { id: 302, text: "Press Start to Continue", image: "", duration: 2, theme: THEMES[6], animation: ANIMATIONS[2].id, align: 'center', fontSize: 'medium', aiVoice: true },
      { id: 303, text: "LEVEL UP NOW", image: "", duration: 2, theme: THEMES[3], animation: ANIMATIONS[0].id, align: 'center', fontSize: 'huge', aiVoice: true }
    ]
  },
  {
    id: 't4',
    ...
    frames: [
      { id: 401, text: "ATTENTION!", image: "", duration: 1, theme: THEMES[1], animation: ANIMATIONS[2].id, align: 'center', fontSize: 'huge', aiVoice: false },
      { id: 402, text: "This offer ends in 24 hours.", image: "", duration: 3, theme: THEMES[1], animation: ANIMATIONS[0].id, align: 'center', fontSize: 'large', aiVoice: true },
      { id: 403, text: "Link in Bio.", image: "", duration: 2, theme: THEMES[1], animation: ANIMATIONS[1].id, align: 'center', fontSize: 'medium', aiVoice: true }
    ]
  }
];
      ...
      <div className="relative w-full max-w-sm bg-neutral-900 border border-white/10 rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 relative aspect-[9/16] w-full bg-black overflow-hidden">
             ...
             <div className={`w-full h-full flex flex-col justify-center p-6 relative transition-colors duration-300 ${frame.theme.bg} ${getAlignmentClass(frame.align)}`}>
                  ...
                  <div className={`relative z-10 w-full break-words px-2 flex flex-wrap gap-x-2 gap-y-1 ${getAlignmentClass(frame.align)}`}>
                    <div className={`${frame.theme.text} drop-shadow-xl ${smartSize}`}>
...
export default function App() {
  ...
  const [isPricingOpen, setIsPricingOpen] = useState(false);
  const [previewTemplate, setPreviewTemplate] = useState(null); // State for Preview Modal
  const [toast, setToast] = useState({ show: false, message: '' });
      ...
      {mode === 'simple' && (
        <main className="flex-1 max-w-7xl mx-auto w-full p-4 lg:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 overflow-hidden lg:h-[calc(100vh-64px)] h-auto">
          ...
          <div ref={previewRef} className="lg:col-span-7 h-auto min-h-[500px] lg:h-full flex flex-col bg-neutral-900/50 rounded-3xl border border-white/5 p-4 lg:p-8 relative order-1 lg:order-2 scroll-mt-20">
              ...
              <div className="flex-1 flex items-center justify-center overflow-hidden">
                <div onClick={togglePlay} className={`relative w-full transition-all duration-500 ease-in-out bg-black rounded-[2rem] border-4 border-gray-800 shadow-2xl overflow-hidden flex flex-col cursor-pointer ${ASPECT_RATIOS[selectedRatio].containerClass} ${isPlaying ? 'hover:scale-[1.02] active:scale-[0.98]' : ''}`}>
                  {/* Smart Container with Flex Wrapping */}
                  <div className={`flex-1 w-full h-full flex flex-col items-center justify-center p-6 text-center transition-colors duration-300 relative ${currentFrameIndex >= 0 && !activeFrame.image ? activeTheme.bg : 'bg-black'} ${currentFrameIndex >= 0 ? getAlignmentClass(activeFrame.align) : ''}`}>
                    ...
                    {currentFrameIndex >= 0 ? (
                      <div className={`relative z-10 w-full break-words px-4 flex flex-wrap gap-x-3 gap-y-1 ${getAlignmentClass(activeFrame.align)}`}>
                        {/* Word Wrapping Logic */}
                        <div className={`${activeTheme.text} drop-shadow-xl ${smartFontSize}`}>
                          {activeFrame.text.split(' ').map((word, idx) => (
                            <span key={idx} className={`inline-block word-hidden ${idx % 2 !== 0 ? activeTheme.accent : ''} ${activeAnim}`} style={{ animationDelay: `${idx * 0.25}s` }}>{word}&nbsp;</span>
                          ))}
                        </div>
                      </div>
                    ) : (
      ...
      {mode === 'custom' && (
        <div className="flex-1 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
           ...
           <div className="flex-1 bg-[#121212] relative flex items-center justify-center p-4">
              <div onClick={togglePlay} className={`relative shadow-[0_0_50px_rgba(0,0,0,0.5)] cursor-pointer transition-all duration-300 bg-black rounded-lg overflow-hidden border border-white/10 ${ASPECT_RATIOS[selectedRatio].containerClass} ring-1 ring-white/10`}>
                 <div className={`w-full h-full flex flex-col justify-center p-6 relative ${activeTheme.bg} ${getAlignmentClass(activeFrame.align)}`}>
                    ...
                    <div className={`relative z-10 w-full px-4 flex flex-wrap gap-x-2 gap-y-1 ${getAlignmentClass(activeFrame.align)}`}>
                       <div className={`${activeTheme.text} drop-shadow-xl ${smartFontSize}`}>
                          {activeFrame.text.split(' ').map((word, idx) => (
                            <span key={idx} className={`inline-block ${idx % 2 !== 0 ? activeTheme.accent : ''} ${activeAnim}`} style={{ animationDelay: `${idx * 0.2}s` }}>{word}&nbsp;</span>
                          ))}
                       </div>
                    </div>
                    ...
                    {!isPlaying && (
                      <div className="absolute inset-0 flex items-center justify-center bg-black/20 backdrop-blur-[1px]">
                        <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-md border border-white/30"><Play className="fill-white text-white ml-1" /></div>
                      </div>
                    )}
                 </div>
              </div>
                          ...
                          <textarea 
                           ...
                           <div className="grid grid-cols-2 gap-4">
                              <div className="bg-neutral-900 p-3 rounded-xl border border-white/10 flex items-center justify-center">
                                 <div className="text-xs text-gray-400 text-center">Font Size: <span className="text-white font-bold">Auto-Adjust</span> based on length</div>
                              </div>
